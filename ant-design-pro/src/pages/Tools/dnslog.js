...
@connect(({ rule_fetchdnslog, loading }) => ({
  rule_fetchdnslog,
  loading: loading.models.rule_fetchdnslog,
}))
@Form.create()
    ...
    },
    {
      ...
      render: (text, record) => (
        <Fragment>
          ...
          <a onClick={() => this.updateVulnStatus(true, record, verify=1)}>确认</a>
          <Divider type="vertical" />
          <a onClick={() => this.updateVulnStatus(true, record, verify=2)}>忽略</a>
          {/* <a href="">订阅警报</a> */}
        </Fragment>
      ),
    },
            ...
            <StandardTable
              ...
              expandedRowRender={record =>
                  <div style={{ margin: 0, padding: 0 }}>
                    <span style={{ margin: 0, whiteSpace: 'pre-wrap', wordWrap: 'break-word', wordBreak: 'break-all'}}>路径:   {record.path}</span>
                    <Divider />
                    <span style={{ margin: 0 }}>请求方法: {record.method} 
                    {/* <Divider type="vertical" /> 漏洞更新时间: {record.modified}  */}
                    <Divider type="vertical" /> UA: {record.http_user_agent}</span>
                  </div>
                }
